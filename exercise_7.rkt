...
(define-struct (player game-object)
  ...
  (define (update! p) (when firing-engines? (set-game-object-velocity! p
                                               (posn-+ (game-object-velocity p)
                                                   (forward-direction p)))))
  
  ;; render: player -> image
  ;; Draw the player's ship
  (define (render p) (isosceles-triangle 30 40 "solid" "aquamarine"))
...
(define-struct (asteroid game-object)
  ...
  ;; Draw the asteroid
  (define (render a) (circle (asteroid-radius a) "solid" (asteroid-color a)))
  ;; radius: asteroid -> number
  ;; Size of the asteroid
  (define (radius a) (asteroid-radius a)))
...
(define-struct (missile game-object)
  ...
  ;; Draw the missile
  (define (render m) (circle 1 "solid" "white"))
...
(define-struct (heat-seeker missile)
  ...
  (define (update! m)
    (set-game-object-velocity! m
                              (posn-+ (game-object-velocity m)
                              (posn-* 45 (heading-of (closest-asteroid-to m)
                                          m)))))
...
()
 ...
 ;; render: ufo -> image
  ;; Draw the ufo
  (define (render u) (square 40 "solid" "red"))
  ;; radius: ufo -> number
  ;; Size of the ufo
(define (radius u) 60)
...
(define (update! u)
  (set-game-object-velocity! u (posn-+ (game-object-velocity u)
                                 (posn-* 1 (heading-of the-player u))))))
