...
(define (ancestors-names-inclusive pers)
    (cond [(empty? pers) empty]
          [else (cons (human-name pers) (append (ancestors-names-inclusive (human-parent-1 pers))
...
(define (related? a1 a2)
    (ormap (λ (str) (member? str (ancestors-names-inclusive a2)))
...
(define (list-contents tree)
  (cond [(empty? tree) empty]
        [else (append (append (list-contents (node-smaller tree)) (list (person-ssn (node-person tree))))
